...
struct IntroClassicView: View {
    ...
    private func finish() {
        ...
        finished = true
        sequence?.cancel()
        // The reveal track is deliberately *not* stopped. The player lives on
        // `SoundEffects`, not on this view, so it keeps going once the intro
        // has handed over — the swell resolves under whatever comes next
        // instead of being cut off mid-phrase.
